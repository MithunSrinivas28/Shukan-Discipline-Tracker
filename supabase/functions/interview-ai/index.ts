// Interview Lab AI endpoint: generates next interviewer turn, per-question evaluation, and final report.
const corsHeaders = {
...
function systemPrompt(domain: string, customTopic: string | null, difficulty: string) {
...
Deno.serve(async (req) => {
  ...
  try {
    const { action, domain, customTopic, difficulty, transcript, lastQuestion, lastAnswer } =
      await req.json();
    const sys = systemPrompt(domain, customTopic, difficulty);
